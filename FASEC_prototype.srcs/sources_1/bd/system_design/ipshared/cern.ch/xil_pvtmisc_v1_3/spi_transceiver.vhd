...
architecture rtl of spi_transceiver is
  ...
  constant c_CSNHIGH      : natural := 4;       -- additional sclk pulses to
                                                -- extend cs_n high pulse
...
begin
  ...
  assert (c_CSNHIGH<g_DATA_WIDTH)
    report "c_CSNHIGH should be smaller than g_DATA_WIDTH!"
    severity error;
  --=============================================================================
          ...
          when rx =>
            if s_counter = s_act(g_CLOCK_DIVIDER downto 0) then
              if v_cnt = g_DATA_WIDTH then
                s_state <= stop_cs;
                v_cnt := 0;     -- reuse in stop_cs state
              else
                s_state <= tx;
                -- shift on edge only
