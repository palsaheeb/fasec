...
#false path constraint for select line of clock mux
set_false_path -to [get_pins -hier -filter { name =~ */sgmii_logi*/clock_generation/i_bufgctrl_sgmii_clk/S*} ]
  #clock defined for input of clock multiplexer in 10/100 mbps modes. period defined here is 62.5*2/5 = 25 MHz 
  #This is the worst case for 100 and 10 mbps modes.
  
  create_generated_clock -name clk_25 -source [get_pins -hier -filter {name =~  *transceiver_inst/gtwizard_inst/*/gtwizard_i/gt0_GTWIZARD_i/gt*e2_i/TXOUTCLK}] -divide_by 5 -multiply_by 2 [get_pins -hier -filter { name =~  */sgmii_logic/clock_generation/i_bufgctrl_sgmii_clk/I1}]
  create_generated_clock -name xfr_clk -source [get_pins -hier -filter {name =~  *transceiver_inst/gtwizard_inst/*/gtwizard_i/gt0_GTWIZARD_i/gt*e2_i/TXOUTCLK}] -divide_by 1 -multiply_by 2 [get_pins -hier -filter { name =~  */sgmii_logic/clock_generation/i_bufgctrl_sgmii_clk/I0}]
  

  set_clock_groups -physically_exclusive -group [get_clocks clk_25] -group [get_clocks xfr_clk] 
  set_propagated_clock [get_clocks xfr_clk]
  set_propagated_clock [get_clocks clk_25]
